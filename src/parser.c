...
static void
error_at(parser_t *parser, const char *message)
{
...
parser_advance(parser_t *parser)
{
	...
	while (true) {
		...
		fprintf(stderr, "[%u:%u] Lexer error at '%.*s'\n", parser->current.line,
			parser->current.col, (int)parser->current.length, parser->current.start);
		parser->had_error = true;
	}
}
...
static bool
parser_check(parser_t *parser, tokentype_t type)
{
	return parser->current.type == type;
}

static bool
parser_match(parser_t *parser, tokentype_t type)
{
...
static bool
parser_expect(parser_t *parser, tokentype_t type, const char *error_message)
{
	if (parser_check(parser, type)) {
...
parse_reg(token_t token)
{
	return token.reg_id;
}

static inline int
parse_imm(token_t token)
{
	return token.int_value;
}

static uint32_t
parse_s_type(parser_t *parser, const isa_entry_t *inst)
{
...
static uint32_t
parse_jalr(parser_t *parser, const isa_entry_t *inst)
{
	parser_expect(parser, TOK_REGISTER, "Expected register");
...
static uint32_t
parse_alu_imm(parser_t *parser, const isa_entry_t *inst)
{
	parser_expect(parser, TOK_REGISTER, "Expected register");
	uint8_t rd = parse_reg(parser->previous);
...
static uint32_t
parse_load(parser_t *parser, const isa_entry_t *inst)
{
	parser_expect(parser, TOK_REGISTER, "Expected register");
	uint8_t rd = parse_reg(parser->previous);
...
static uint32_t
parse_b_type(parser_t *parser, const isa_entry_t *inst)
{
	parser_expect(parser, TOK_REGISTER, "Expected register");
	uint8_t rs1 = parse_reg(parser->previous);
...
static uint32_t
parse_j_type(parser_t *parser, const isa_entry_t *inst)
{
	parser_expect(parser, TOK_REGISTER, "Expected register");
	uint8_t rd = parse_reg(parser->previous);
...
static uint32_t
parse_i_type(parser_t *parser, const isa_entry_t *inst)
{
	switch (inst->opcode) {
	case OPCODE_ALU_IMM:
...
static void
parse_r_type(parser_t *parser, const isa_entry_t *inst)
{
	parser_expect(parser, TOK_REGISTER, "Expected register");
	uint8_t rd = parse_reg(parser->previous);
	...
	instruction_t parsed_instr =
		make_r_instr(inst->opcode, rd, rs1, rs2, inst->funct3, inst->funct7);
	i_emit(&parsed_instr);
}

static void
parse_label(parser_t *parser)
{
...
static void
parse_directive(parser_t *parser)
{
...
static void
parse_inst(parser_t *parser)
{
	const isa_entry_t *inst = find_instruction(parser->current.start, parser->current.length);
	if (inst == NULL) {
	...
	switch (inst->format) {
	case R_TYPE:
		parse_r_type(parser, inst);
	case I_TYPE:
		parse_i_type(parser, inst);
	case S_TYPE:
		parse_s_type(parser, inst);
	case B_TYPE:
		parse_b_type(parser, inst);
	case U_TYPE:
		assert(0 && "U-type parsing not implemented yet");
	case J_TYPE:
		parse_j_type(parser, inst);
	default:
