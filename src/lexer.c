...
static uint8_t find_register(const char* text, size_t length) {
...
static token_t make_token(lexer_t* lexer, tokentype_t type, size_t start_offset,
                          uint32_t start_col, uint32_t start_line) {
  return (token_t){
      .type = type,
      .line = start_line,
      .col = start_col,
...
static token_t make_register_token(lexer_t* lexer, size_t start_offset,
                                   uint32_t start_col, uint32_t start_line,
                                   uint8_t register_id) {
  return (token_t){
      .type = TOK_REGISTER,
...
static token_t make_int_token(lexer_t* lexer, size_t start_offset,
                              uint32_t start_col, uint32_t start_line) {
  return (token_t){
      .type = TOK_INTEGER,
      .line = start_line,
...
static void read_identifier(lexer_t* lexer) {
  while (is_alpha(peek(lexer)) || is_digit(peek(lexer))) {
    advance(lexer);
  }
}

static bool is_instruction(const char* text, size_t length) {
...
static token_t classify_name(lexer_t* lexer, size_t start_offset,
                             uint32_t start_col, uint32_t start_line) {
  ...
  if (is_instruction(text, length)) {
    return make_token(lexer, TOK_INSTRUCTION, start_offset, start_col,
                      start_line);
  }
  ...
  if (register_id != UNKNOWN_REGISTER_ID) {
    return make_register_token(lexer, start_offset, start_col, start_line,
                               register_id);
  }
...
token_t lexer_next_token(lexer_t* lexer) {
  ...
  switch (c) {
    ...
    case '\n':
      return make_token(lexer, TOK_NEWLINE, start_offset, start_col,
                        start_line);
    case ':':
